...
begin	
		if exists(select 1 from tbViagem where IdViagem = p_IdViagem) then
			insert into tbTransporte(TipoTransporte, CodigoTransporte, IdViagem,Companhia)
			values(p_TipoTransporte, p_CodigoTransporte,p_IdViagem, p_Companhia);
		end if;
end $$
delimiter ;
call InserirTransporte('Ônibus', 123, 1,'Gol');
call InserirTransporte('Avião', 1234,2, 'Latam');
call InserirTransporte('Ônibus', 12,3, 'Cometa');
call InserirTransporte('Avião', 12345,4, 'Azul');
call InserirTransporte('Ônibus', 053, 5,'EMTU');
call InserirTransporte('Avião', '123A',5, 'Azul');
